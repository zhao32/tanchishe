...
export default class ScrollViewList extends cc.Component {
    ...
    private _slideMode = 1;
    @property({
        tooltip: "滑动模式： 1, 普通；2, 粘附模式，将强制关闭滚动惯性；3, 页面模式，将强制关闭滚动惯性",
    })
    set slideMode(value) {
        if (value != null)
    ...
    @property({
        type: cc.Component.EventHandler,
        tooltip: "滚动到底部，分页列表获取下一页",
    })
    nextPageEvent = new cc.Component.EventHandler();

    @property({
        tooltip: "选择模式: 0: 无选择, 1: 单选,2 多选",
    })
    ...
    set numItems(val) {
        ...
        if (t._numItems > val && t._customSize) {
            t._customSize = {};
        }
    ...
    private _actualNumItems = 0;
    private goBottomTween;
    private _cyclicNum;
    private nearestListId;
    private firstListId;
    private displayItemNum;

    private _itemTmp = null;
    private _pool = null;
    private _aniDelRuning = false;
    private _aniDelItem = null;
    private _aniDelBeforePos = null;
    private _aniDelBeforeScale = 1;
    private _aniDelCB = null;
    ...
    private _lineGap;
    private _colLineNum;
    private _verticalDir;
    private _horizontalDir;
    ...
    private _updateDone;
    private curPageNum;
    private _alignCalcType;

    private _itemSize;
    private _needUpdateWidget;
    private multSelected;
    private _sizeType;
    //初始化各种..
    _init() {
        let t = this;
        ...
        t._gobottomNode = this.node.getChildByName('gobottomNode');
        t._viewMask = this.node.getChildByName('view');
    ...
    private _customSize;
    private _allItemSize;
    private _allItemSizeNoEdge;
    private _cyclicPos1;
    private _cyclicPos2;
    private _cyclicAllItemSize;
    private _cycilcAllItemSizeNoEdge;
    private _lack;
    ...
    private frameCount = null;
    private viewTop;
    private viewBottom;
    private viewRight;
    private viewLeft;
    private _doneAfterUpdate;
    //滚动进行时...
    _onScrolling(ev?) {
    ...
    private elasticLeft;
    private elasticRight;
    private elasticTop;
    private elasticBottom;
    //计算View位置
    _calcViewPos() {
        let scrollPos = this.content.getPosition();
        switch (this._alignCalcType) {
    ...
    private _beganPos;
    private curScrollIsTouch;
    private scrollToListId;
    private adhering;
    private _scrollItem;
    private _scrollPos;
    private _adheringBarrier;
    //滚动开始时..
    _onScrollBegan() {
    ...
    _onScrollEnded() {
        ...
        //滚动到最底部
        let endData = this.displayData[this.displayData.length - 1];
        if (endData && endData.id == this._numItems - 1) {
            cc.Component.EventHandler.emitEvents([this.nextPageEvent]);
        }
        ...
        // if (this._gobottomNode && this.displayData[this.displayData.length - 1].id < this._numItems) {
        //     this._gobottomNode.active = true;
        //     console.log('yyy', this.displayData[this.displayData.length - 1].id, this._numItems);
        // }
    }
    // 触摸时
    ...
    _onItemAdaptive(item) {
        if (
            ...
            if (!this._customSize)
                this._customSize = {};
            //cc.log('yyy item.height', this._itemSize.height, item.height);
    ...
    updateEndY(nid) {
        this.displayData
        let displayItemNum = this.displayData.length;
        for (let c = 0; c < displayItemNum; c++) {
            if (this.displayData[c].id <= nid) continue;
            this.displayData[c] = this._calcItemPos(this.displayData[c].id);
        }
    }
    ...
    _createOrUpdateItem(data) {
        ...
        if (!item) { //如果不存在
            ...
            else if (this._customSize && !this._customSize[item._listId]) {
                item.setContentSize(this._itemSize);
            }
            ...
            let listItem = item.getComponent(ScrollViewListItem);
            ...
            if (this.renderEvent) {
                //   console.log('yyy更新一个item', this._actualNumItems, this._numItems);
            ...
            item.setPosition(cc.v2(data.x, data.y));
            if (this.renderEvent && this.qiangzhi) {
                cc.Component.EventHandler.emitEvents([this.renderEvent], item, data.id % this._actualNumItems);
            }
        }
        //y this._resetItemSize(item);
        ...
        if (this._virtual) {
            ...
            for (let n = arr.length - 1; n >= 0; n--) {
                ...
                if (this._scrollItem && item._listId == this._scrollItem._listId)
                    continue;
                item.isCached = true;
                item.getComponent('bagBaseItem') && item.getComponent('bagBaseItem').setNull();
                this._pool.put(item);
     ...
     */
    scrollTo(listId, timeInSecond, offset = 0, overStress = false) {
        let t = this;
        if (!t.checkInited())
            return;
    ...
    setNumItems(num) {
        let t = this;
        t._numItems = num;
        t._actualNumItems = num;
        t._resizeContent();
    }
    /**
     * @param {int} num  划到指定位置
     */
    viewGotoNum(num, allNum = 0) {
        let t = this;
        t.goBottomTween = true;
        t.numItems = allNum || t._numItems;
        t.goBottomTween = false;
        if (!t.checkInited())
            return;
        if (!t._virtual && t._layout && t._layout.enabled)
            t._layout.updateLayout();

        this.viewBottom = -t.content.height;
        let bottom = (this._itemSize.height + this._lineGap) * num;
        let top = Math.max((bottom - this.node.height), 0);
        this.content.setPosition(0, top);
        this._onScrolling();
        this.scrollTo(num, 0.2);
        // t.goBottomTween = false;
    }
    /**
     * 
    ...
    viewGoBottom(num, time = 0.2) {
        ...
        this.viewBottom = -t.content.height;
        this.viewTop = this.viewBottom + this.node.height;
        this.content.setPosition(0, -this.viewTop);
        this._onScrolling();
        this.scrollTo(num, time);
    }
    /**
     * 设置content的位置在最高点
     * 重新排版 item 位置
    */
    goBottom() {
        ...
        this._calcNearestItem();
        this.scrollTo(this._numItems, 0.2);
    }
    /**
     * 计算当前滚动窗最近的Item
     */
    ...
    getContentdY() {
        return this._scrollView.getMaxScrollOffset().y - this._scrollView.content.y;
    }
    startCustomSize() {
        this._customSize = {};
    }
    setitemHeight(val = 35, isWorld = false, bottomN = 0) {
        this._itemSize && (this._itemSize.height = val);

        //this._topGap =//顶边距
        let bottomGap = bottomN || (val > 50 ? 50 : 0);
        let higt = isWorld ? 485 : 535;
        this.node.height = higt - bottomGap;
        this.node.y = -274 + bottomGap;
        this._viewMask && (this._viewMask.height = higt - bottomGap) && (this._viewMask.y = higt - bottomGap);
    }
}
